/* PIN login — bcrypt hash + JWT cookie.
   PIN is never stored in plain text; only the hash is stored in .env. */
...
/* brute-force guard — 15 min lock after 5 failed attempts from an IP */
...
/* Storing hash in base64 prevents shell / env variable interpolation issues with '$' */
function readHash() {
  const raw = (process.env.SHOP_PIN_HASH || '').trim().replace(/^['"]|['"]$/g, '');
  if (!raw) return null;
  if (raw.startsWith('$2')) return raw;                 // plain bcrypt
  try {                                                  // base64
