/* Neon Postgres connection and database queries.
   Every function uses shop_id from the server session — never trusted from the browser. */

import { neon, neonConfig } from '@neondatabase/serverless';

/* Lazy connection — connects on-demand upon request */
let _sql = null;
function conn() {
  if (!_sql) {
    if (!process.env.DATABASE_URL) throw new Error('DATABASE_URL_MISSING');
    // Local proxy support if configured
...
export async function addPayment(o) {
  const [p] = await sql`
    ...
    returning *`;
  return p;   // FIFO view automatically calculates remaining bill due
}
...
/* Item-wise monthly profit breakdown */
