...
export async function updateShop(o) {
  const [s] = await sql`
    update shop set
      ...
      overdue_days = coalesce(${o.overdue_days}, overdue_days),
...
export async function addItem(o) {
  try {
    const [i] = await sql`
      ...
      values (${SHOP_ID}, ${o.code}, ${o.barcode || null}, ${o.name}, ${o.category || 'अन्य'},
...
export async function createSale(o) {
  ...
  const [sale] = await sql`
    ...
    values (${SHOP_ID}, ${billNo.no}, ${o.customer_id || null},
            ${o.customer_name || 'नकद ग्राहक'}, ${o.bill_date || 'today'},
...
export async function weeklyReport() {
  ...
  const overdue = await sql`
    select id, name, mobile, balance, days_overdue
    from debtors where shop_id = ${SHOP_ID} and days_overdue >= ${odDays}
    order by balance desc`;
  const big = await sql`
    select id, name, balance, credit_limit
    from debtors where shop_id = ${SHOP_ID}
      and credit_limit > 0 and balance > credit_limit order by balance desc`;
